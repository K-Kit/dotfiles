<!-- Canonical source: ai-safety-plugins/plugins/research/agents/references/ci-standards.md — keep in sync. This copy is NOT byte-identical: it differs in exactly two intentional ways, this comment and the "Confidence intervals, not continuous integration" disambiguation prefixed to the lede below (needed because "CI" reads as continuous integration in a docs/ directory). Everything after that is in sync as of 2026-07-27. Keep this copy as the pointer target for always-loaded rules: it is profile-independent, whereas the canonical one sits inside the research plugin, which is off in most sessions. To check for drift, run from the repo root — empty output means in sync, and any output is real drift, not the two known deltas: diff <(sed -e '1{/^<!-- Canonical source:/d}' -e '2{/^$/d}' -e 's/^\*\*Confidence intervals\*\*, not continuous integration\. //' claude/docs/ci-standards.md) ~/code/marketplaces/ai-safety-plugins/plugins/research/agents/references/ci-standards.md -->

# Confidence Interval Standards for Experiment Design and Analysis

**Confidence intervals**, not continuous integration. Canonical reference for statistical reporting in all research experiments.
