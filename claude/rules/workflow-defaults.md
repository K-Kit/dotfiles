...
Run experiments and compute-heavy jobs through `jexp` (Pueue with systemd cgroup caps from `config/resources.conf`), not bare `uv run python` — direct runs have no resource limits and can starve the machine. `pueue` and `systemctl` are in `excludedCommands`, so they bypass the sandbox. If `systemd --user` is unavailable: `loginctl enable-linger $(whoami)`. If memory limits are silently ignored: `sudo systemctl set-property user-$(id -u).slice Delegate=yes`.
