...
- Hide idle apps (macOS only, **off by default** — enable with `--hide-idle-apps`) - Polls every `HIDE_IDLE_POLL_SECONDS` (launchd `StartInterval`, default 60) and walks each covered-up app along a **three-rung ladder**: hide (Cmd+H equivalent, via System Events `visible`) after `hide_after` minutes covered, close its windows after a further `close_after`, quit it after a further `quit_after` (defaults 15 / 15 / 30). Per-app policy lives in `config/app-lifecycle.yaml` (which replaced `config/clear_mac_apps.conf`) and is read by both this job and `clear-mac-apps` through `custom_bins/app-lifecycle-config`, so neither zsh script parses YAML; only the poll interval stays in `config/hide-idle.conf`, because it has to match the launchd `StartInterval` or gap detection means nothing. Env vars override both (`HIDE_AFTER_MINUTES`, `CLOSE_AFTER_MINUTES`, `QUIT_AFTER_MINUTES`, `USER_IDLE_MINUTES`, `HIDE_IDLE_MIN_VISIBLE_PERCENT`, `HIDE_IDLE_POLL_SECONDS`) — `hide-idle-apps --help` lists them.
- Hide-idle ladder semantics - Each app carries `manual:` (what the macOS Shortcut → `clear-mac-apps` does to it right now) and `auto:` (what the idle job may do), on the ordered scale `skip` < `hide` < `close` < `quit`. The ladder stops at **`min(manual:, auto:)`**, but that ceiling binds only the destructive rungs: **hiding is governed by `auto:` alone**, and `auto: skip` is the only exemption from being hidden — it is what the old `[hide-idle-exclude]` section became. So `{manual: skip}` (Obsidian, Things) is hidden but never closed or quit; `{manual: close}` (Spotify) is hidden and closed but never quit; an app the file never mentions takes the defaults and goes all the way. The destructive rungs additionally require the **machine to be idle**: no HID input for `USER_IDLE_MINUTES` (`ioreg -c IOHIDSystem` `HIDIdleTime`) and no live microphone (a `coreaudiod` power assertion listing `audio-in`). Busy **pauses** those clocks rather than resetting them, and **either signal being unreadable counts as busy** — "cannot tell whether the mic is live" must never license closing windows. Escalation is not reimplemented here: the job calls `clear-mac-apps --only "<App>" [--max-action close]`, so config vetoes apply to both triggers through one code path (`--only` filters before classification, `--max-action` caps after it). **At most one destructive action per poll**, most overdue first, so a slow quit can't overrun the interval.
- Hide-idle visibility and state - Visibility comes from `tools/window-exposure/main.swift`, a compiled-once Swift helper reading `CGWindowListCopyWindowInfo` and computing, per window, the fraction of its frame not covered by the **union** of the opaque, different-PID windows in front of it (exact, via coordinate compression; single-window containment kept as a fast path). Because it is a union, several windows jointly covering one are detected. **The frontmost app is never hidden**, whatever its coverage, and an app with no window in the onscreen list (another Space, minimised) is *unknown* rather than not-exposed, so it is never hidden either. Hiding also requires two consecutive covered polls; any invalid poll (config unreadable, state from an older version, helper failure, empty window list, or a gap >3× the interval, e.g. after sleep) acts on nothing and restarts all timers. `hide-idle-apps --dry-run` prints one resolved decision per app — measured visibility, current rung, the app's `manual:`/`auto:` values, the rung it is heading for, and what is holding it there (`frontmost`, `visible enough`, `no window here`, `streak 1/2`, `in 10m`, `busy`, or `ceiling`) — plus whether the machine is currently busy. That is the way to calibrate against a real layout: visibility alone answers "is the threshold right", and the gate column answers "why is this app still sitting here", which for a paused clock is otherwise indistinguishable from a stuck one. `HIDE_IDLE_MIN_VISIBLE_PERCENT` is a whole percent (40, not 0.4, which is valid and means 0.4%); an out-of-range value hides nothing and logs the reason to stderr rather than falling back to a default. A small OS-chrome set (Finder, Dock, SystemUIServer, loginwindow, WindowServer) is hardcoded-excluded in `custom_bins/hide-idle-apps`, not user-configurable. Per-PID state in `~/.cache/hide-idle-apps/state`, format `#v3`, now carrying each app's rung and a *pause-adjusted* clock epoch (`now - phase_clock_epoch` reads elapsed **unpaused** seconds); an older state file is discarded and rebuilt, never migrated. The phase field exists because exposure alone cannot tell "on another Space" from "hidden by us" — neither owns an onscreen window. An app that becomes frontmost, or owns an onscreen window again, drops back to the bottom rung, which is also what self-heals a hide or close that silently failed; dropping back resets the exposure timer as well as the phase clock, so an app the user un-hides is not hidden again in the same second. That self-heal cannot rescue a *hidden* app, which owns no onscreen window by construction — so a rung is advanced before the call (a job killed mid-close must not re-fire every poll) but **given back if the call returns nonzero**, or a close that failed would start the quit clock.
- Closing windows without stealing focus - `clear-mac-apps` closes windows by clicking each window's `AXCloseButton` via System Events, so it no longer has to make the app frontmost and type Cmd+W at it. Apps whose windows have no such button fall back to the old keystroke path (Cmd+W, escalating to Cmd+Shift+W for tabbed apps) and are remembered in `~/.cache/hide-idle-apps/ax-capability`, keyed by app name, so the probe happens at most once a month. An app with **no** windows is skipped entirely and never recorded — a probe that fails for want of a window says nothing about what the app supports. A missing close button fails the click immediately, but a *slow* close is not held against the app: the probe only gives up after `patience` (default 3) **consecutive** clicks leave the window count unmoved, because a single slow teardown would otherwise brand a capable app keystroke-only until the record expires. Records are `name<TAB>epoch` and are **trusted for `AX_CAPABILITY_TTL_DAYS` (default 30)**, after which the app is probed again and the record rewritten in place — the record is only consulted when one exists, so a permanent record meant an app that *gained* a close button in an update was typed at forever. A record with no timestamp predates expiry and counts as stale.
