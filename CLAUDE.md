...
- Hide idle apps (macOS only, **off by default** — enable with `--hide-idle-apps`) - Polls every `HIDE_IDLE_POLL_SECONDS` (launchd `StartInterval`, default 60) and hides (Cmd+H equivalent, via System Events `visible`) apps whose every window has stayed **less than `HIDE_IDLE_MIN_VISIBLE_PERCENT` visible** (default 40) for `HIDE_IDLE_MINUTES` (default 15). All three tunables live in `config/hide-idle.conf`; env vars of the same name override it. Visibility comes from `tools/window-exposure/main.swift`, a compiled-once Swift helper reading `CGWindowListCopyWindowInfo` and computing, per window, the fraction of its frame not covered by the **union** of the opaque, different-PID windows in front of it (exact, via coordinate compression; single-window containment kept as a fast path). Because it is a union, several windows jointly covering one are detected. **The frontmost app is never hidden**, whatever its coverage, and an app with no window in the onscreen list (another Space, minimised) is *unknown* rather than not-exposed, so it is never hidden either. Hiding also requires two consecutive covered polls; any invalid poll (helper failure, empty window list, or a gap >3× the interval, e.g. after sleep) hides nothing and restarts all timers. `hide-idle-apps --dry-run` prints each app's measured visibility beside its name — the way to calibrate the threshold against a real layout. `HIDE_IDLE_MIN_VISIBLE_PERCENT` is a whole percent (40, not 0.4, which is valid and means 0.4%); an out-of-range value hides nothing and logs the reason to stderr rather than falling back to a default. Exclusions: `config/clear_mac_apps.conf`'s `[hide-idle-exclude]` section, opt-out (hide-by-default) and orthogonal to `[no-touch]` (which only means "don't close/quit"); a small OS-chrome set (Finder, Dock, SystemUIServer, loginwindow, WindowServer) is hardcoded-excluded in `custom_bins/hide-idle-apps`, not user-configurable. Per-PID state in `~/.cache/hide-idle-apps/state`.
